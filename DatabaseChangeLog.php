...
class DatabaseChangeLog
{
    ...
    private function checkTableConfig($data)
    {
        ...
        if(!array_key_exists($data['action'],$this->logTablesConfig[$data['table']])){
            return false;
        }
     ...
     * @return int groupId
     */
    private function logValueUpdate($value,$logData,$groupId=0)
    {
    ...
    private function logUpdate($parsed)
    {
        ...
        foreach ($parsed['SET'] as $value){
            $groupId = $this->logValueUpdate($value,$logData,$groupId);
        }
